...
contract StorableMsgHelperTest is Test {
    ...
    function test_ToHash_Works_EmptyMessage() public view {
        require(
            EMPTY_STORABLE_MESSAGE.toHash() ==
                StorableMsgHelper.EMPTY_STORABLE_MESSAGE_HASH,
    ...
    function test_applyType_TopDown() public pure {
        ...
        require(
            storableMsg.applyType(
                SubnetID({root: ROOTNET_CHAINID, route: from})
            ) == IPCMsgType.TopDown,
            "Should be TopDown"
        );
        ...
        require(
            storableMsg.applyType(subnetId) == IPCMsgType.TopDown,
            "Should be TopDown"
        );
        ...
        require(
            storableMsg.applyType(
                SubnetID({root: ROOTNET_CHAINID, route: current2})
            ) == IPCMsgType.TopDown,
            "Should be TopDown"
        );
    }
    ...
    function test_applyType_BottomUp() public pure {
        ...
        require(
            storableMsg.applyType(
                SubnetID({root: ROOTNET_CHAINID, route: to})
            ) == IPCMsgType.BottomUp,
            "Should be BottomUp"
        );
    }
