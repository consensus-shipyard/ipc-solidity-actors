...
contract SubnetIDHelperTest is Test {
  ...
  function test_Equals_Works_NonEmpty() public view {
    ...
    require(
      SubnetID({root: ROOTNET_CHAINID, route: route}).equals(SubnetID({root: ROOTNET_CHAINID, route: route})) == true
    );
    require(
      SubnetID({root: ROOTNET_CHAINID, route: route}).equals(SubnetID({root: ROOTNET_CHAINID, route: route2})) == false
    );
  }
