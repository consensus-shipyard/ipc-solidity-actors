...
library CrossMsgHelper {
    ...
    function execute(CrossMsg calldata crossMsg, bytes4 methodSelector) public returns (bool success) {
        bytes memory params = crossMsg.wrapped ? abi.encode(ApplyMsgParams({crossMsg: crossMsg})) : crossMsg.message.params;
        (success, ) = crossMsg.message.to.rawAddress.call{value: crossMsg.message.value}(
