...
library FvmAddressHelper {
    ...
    function isEqual(FvmAddress calldata f1, FvmAddress calldata f2)
        internal
        pure
        returns (bool)
    {
        if (f1.addrType != f2.addrType) { return false; }
    ...
    function isValid(FvmAddress calldata filAddress)
        internal
        pure
        returns (bool)
    {
        require(filAddress.addrType == SECP256K1, "Addr not supported");
        return isValidF1Address(filAddress.payload);
    }

    function isValidF1Address(bytes calldata payload) private pure returns(bool) {
