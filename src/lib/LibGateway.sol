...
library LibGateway {
    ...
    event NewTopDownMessage(bytes32 indexed subnet, uint64 indexed nonce, CrossMsg message);
    ...
    function commitTopDownMsg(CrossMsg memory crossMessage) internal {
        ...
        emit NewTopDownMessage({subnet: subnetId.toHash(), nonce: topDownNonce, message: crossMessage});
    }
